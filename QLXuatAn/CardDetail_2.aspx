<%@ Page Title="" Language="C#" MasterPageFile="~/QLXuatAn/MasterPage.master" AutoEventWireup="true"
    CodeFile="CardDetail_2.aspx.cs" Inherits="QLXuatAn_CardDetail_2" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        jQuery(document).ready(function () {
            ...
            $("#plcInputCardNumber").autocomplete({
                source: function (request, response) {
                    ...
                    $.ajax({
                        ...
                        url: 'CardDetail_2.aspx/getCardNumberByAutocomplete',
                        dataType: "json",
                ...
                if (confirm('Bạn có chắc chắn muốn xóa thẻ phụ này?')) {
                    ...
                    if (_v != undefined && _v != '') {
                        $.ajax({
                            method: "POST",
                            contentType: "application/json; charset=utf-8",
                            url: 'CardDetail_2.aspx/DeleteSubCard',
                            dataType: "json",
                            data: "{ 'subCardId': '" + _v + "'}",
                            success: function (result) {
                                if (result.d == "1") {
            ...
            $("#<%=chbIsLock.ClientID%>").click(function () {

                if ($(this).is(':checked')) {
                    $('#<%=txtLockNote.ClientID%>').show();
                } else {
                    $('#<%=txtLockNote.ClientID%>').val('');
                    $('#<%=txtLockNote.ClientID%>').hide();
                }
            ...
            $("#btnDeleteCustomer").click(function () {
                ...
                if (confirm("Bạn có chắc muốn xóa khách hàng này trên thẻ?")) {
                    $('#<%=hidDelcustomer.ClientID%>').val('1');
            ...
            $("#lnkChangCustomer").click(function () {

                $('#boxAddNewCustomer').show();
                $('#myTab li').removeClass('active');
                $('.tab-content .tab-pane').removeClass('active in');
                $('#myTab li#id_li_customer').addClass('active');
                $('.tab-content #id_card_customer').addClass('active in');
                $('#nav-customer-input').focus();

            });
            $("#AddNewCustomerClose").click(function () {

                $('#boxAddNewCustomer').hide();
                $('#boxAddNewCustomer').val('');
                $('#nav-customer-input').val('');
                $("#btnDeleteCustomer").show();

                if ($('#<%=hidCustomer.ClientID%>').val() != "") {
                    var _id = '<%=ViewState["CustomerID"]%>';

                    $.ajax({
                        type: "POST",
                        contentType: "application/json; charset=utf-8",
                        url: 'CardDetail_2.aspx/getCusInfo',
                        dataType: "json",
                        data: "{ 'cusId': '" + _id + "'}",
                        success: function (result) {
            ...
            $("#nav-customer-input").autocomplete({
                ...
                select: function (event, ui) {
                    //debugger;
                    $('#id_card_customer').find('#<%=hidCustomer.ClientID%>').val(ui.item.Id);
                },
                open: function () {
                    $(this).removeClass("ui-corner-all").addClass("ui-corner-top");
                },
            ...
            $("#btnUpdateRegisterDay").click(function () {
                $.ajax({
                    type: "POST",
                    url: "CardDetail_2.aspx/ChangeRegisterDate",
            ...
            $("#btnUpdateReleaseDay").click(function () {
                $.ajax({
                    type: "POST",
                    url: "CardDetail_2.aspx/ChangeReleaseDate",
            ...
            $('#<%=LinkButton3.ClientID%>').click(function () {
                ...
                $('#<%=txtCustomerName.ClientID%>').val('');
                 $('#<%=chbInactive.ClientID%>').prop('checked', false);
                    ...
                    $('#<%=txtPassword.ClientID%>').val('');
                    $('#<%=id_avatar.ClientID%>').val('');
                return confirm('Lưu ý: Thông tin khách hàng sẽ bị hủy trên thẻ này!');
            });


            //Sự kiện xóa thẻ
            $('#lnkDeleteCard').click(function (e) {
                e.preventDefault();

                var id = $(this).attr("idata");

                if (confirm("Bạn có muốn xóa thẻ này không?")) {

                    AjaxPOST("CardDetail_2.aspx/DeleteCard", '{"id":"' + id + '", "userid":"' + '<%=this.ViewState["UserID"].ToString()%>' + '"}').success(function (result) {
                        if (result.d.toString() == "true") {
                            location.href = '/QLXuatAn/Card.aspx';
                        }
                        else
...
<asp:Content ID="Content4" ContentPlaceHolderID="Content_PageContent" runat="Server">
    <div class="col-xs-12">
        ...
        <div class="alert alert-block alert-success" id="div_alert" runat="server">
        ...
        <form id="frm_CardDetail" class="form-horizontal" runat="server">
            ...
            <div class="tabbable">
                <ul class="nav nav-tabs" id="myTab">
                    <li class="active"><a data-toggle="tab" href="#id_card_info"><i class="green ace-icon fa fa-credit-card bigger-120"></i>Thông tin thẻ </a></li>
                    <li id="id_li_customer"><a data-toggle="tab" href="#id_card_customer"><i class="green ace-icon fa fa-user bigger-120"></i>Khách hàng
                ...
                <div class="tab-content">
                    <div id="id_card_info" class="tab-pane fade in active">
                                ...
                                <div id="boxReleaseDate" runat="server">
                                    <label class="inline">
                                        <%if (ViewState["CardID"] != null)
                                            {
                                                if (ViewState["ChkRelease"].ToString() == "0")
                                                {%>
                                        <input type="checkbox" class="ace" id="ChkReleaseUpdate" runat="server" />
                                        <%}
                                        else
                                        { %>
                                        <input type="checkbox" class="ace" id="ChkRelease" checked="checked" disabled />
                                        <%}
                                        }%>

                                        <span class="lbl blue">Đã phát thẻ</span>
                                    </label>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-offset-3 col-sm-8">
                                <asp:TextBox TextMode="MultiLine" ID="txtLockNote" runat="server" CssClass="form-control" Rows="5" Style="display: none;" placeholder="Lý do khóa thẻ"></asp:TextBox>
                            </div>
                        </div>
                        <%} %>
                    </div>
                    <div id="id_card_customer" class="tab-pane fade">
                        <%if (ViewState["CardID"] != null)
                            { %>
                        <div style="display: none" id="boxAddNewCustomer">
                            <div class="form-group">
                                <label class="col-sm-3 control-label no-padding-right" for="form-field-1">
                                    Khách hàng
                                </label>
                                <div class="col-sm-3 plcCustomerId inline" id="BoxCustomer">
                            ...
                            <asp:HiddenField id="hidCustomer" runat="server"/>--%>
                                    <input type="text" placeholder="Nhập tên hoặc mã khách hàng" class="nav-search-input form-control" id="nav-customer-input" value="<%=ViewState["cusName"] %>" />
                                </div>
                                <div class="col-md-2">
                                    <asp:LinkButton onclick="ChangeCustomer" Text="Lưu lại" runat="server" id="btnAddNewsCustomer" class="btn btn-info btn-success btn-sm"></asp:LinkButton>
                                    <a href="#" class="btn btn-info btn-sm" id="AddNewCustomerClose">Đóng</a>
                                </div>
                                <%-- <div class="col-sm-3">
                        ...
                        <%} %>
                        <input type="hidden" name="hidCustomer" id="hidCustomer" runat="server" value="" onchange="Selection_ChangeCustomer" />
                        ...
                        <div id="id_customer_info">
                            <div class="form-group">
                                ...
                                <div class="col-sm-3">
                                    ...
                                    <%if (ViewState["CardID"] != null && !string.IsNullOrWhiteSpace(ViewState["CustomerID"].ToString()))
                                        { %>
                                    <a href="#" id="btnDeleteCustomer" class="red2"><i class="fa fa-trash-o" aria-hidden="true"></i>Xóa khách hàng</a>
                                    <input type="hidden" id="hidDelcustomer" runat="server" value="0" />
                                    <%} %>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label class="col-sm-3 control-label no-padding-right" for="form-field-1">
                                    Địa chỉ</label>
                                <div class="col-sm-5">
                                    <asp:TextBox ID="txtAddress" runat="server" class="form-control" placeholder="" TextMode="MultiLine" Rows="5" />
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label no-padding-right" for="form-field-1">
                    ...
                    <div id="id_Card_SubCard" class="tab-pane fade">
                        <div class="form-group">
                            ...
                            <div class="col-md-2">
                                <asp:LinkButton runat="server" OnClick="AddSubCard" ID="LinkButton2" class="btn btn-success btn-sm">Thêm</asp:LinkButton>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-md-5">
            ...
            <div class="clearfix form-actions">
                <div class="col-md-offset-3 col-md-9">
                    <%if (ViewState["CardID"] == null)
                        { %>
                    <asp:LinkButton ID="LinkButton1" OnClick="Create" runat="server" class="btn btn-sm btn-info">
                    ...
                    <%if (ViewState["CardID"] != null)
                        { %>
                    <asp:LinkButton ID="LinkButton5" OnClick="Update" runat="server" class="btn btn-sm btn-info">
                            <i class="ace-icon fa fa-check bigger-110"></i>
						    <span class="bigger-110">Lưu lại</span>
                    </asp:LinkButton>
                    <a id="lnkChangCustomer" class="btn btn-sm btn-warning">
                        <i class="ace-icon fa fa-check bigger-110"></i>
                        <span class="bigger-110">Đổi chủ thẻ</span>
                    </a>

                    <asp:LinkButton ID="LinkButton3" OnClick="ReturnCard" runat="server" class="btn btn-sm btn-success">
                            <i class="ace-icon fa fa-check bigger-110"></i>
						    <span class="bigger-110">Trả thẻ</span>
                    </asp:LinkButton>

                    <a id="lnkDeleteCard" idata="<%=ViewState["CardID"] %>" class="btn btn-sm btn-danger">
                            <i class="ace-icon fa fa-check bigger-110"></i>
						    <span class="bigger-110">Xóa thẻ</span>
                    </a>
                    <%} %>
                    <button class="btn btn-sm" type="button" onclick="window.location.href='Card.aspx'">
